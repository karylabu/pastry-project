...
return new class extends Migration
{
    public function up(): void
    {
        ...
        foreach ($recipes as $slug => $lines) {
            ...
            $recipeId = DB::table('cake_recipes')->where(['flavor_id' => $flavorId, 'base_size_id' => $baseSizeId])->value('id');
            if (!$recipeId) {
            ...
            if (DB::table('cake_recipe_ingredients')->where('recipe_id', $recipeId)->exists()) continue;
            foreach ($lines as [$name, $quantity, $unit]) {
                DB::table('cake_recipe_ingredients')->insert([
                    'recipe_id' => $recipeId,
                    'ingredient_id' => $ingredientId($name, $unit),
                    'quantity' => $quantity,
                    'unit' => $unit,
                    'created_at' => now(),
                    'updated_at' => now(),
    ...
    {
        // Initial recipe data is intentionally preserved to avoid deleting admin edits.
    }
};
